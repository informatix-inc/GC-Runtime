...
MachOper *State::MachOperGenerator(int opcode, Compile* C){
  ...
  case INDPOSINDEXOFFSET:
    return new (C) indPosIndexOffsetOper(_kids[1]->_leaf->get_long() );
  ...
  case INDPOSINDEXOFFSETNARROW:
    return new (C) indPosIndexOffsetNarrowOper(_kids[1]->_leaf->get_long() );
...
MachNode *State::MachNodeGenerator(int opcode, Compile* C){
  ...
  case leaPPosIdxOff_rule: {
      leaPPosIdxOffNode *node = new (C) leaPPosIdxOffNode();
      node->_bottom_type = _leaf->bottom_type();
      return node;
    }
  ...
  case leaPPosIdxOffNarrow_rule: {
      leaPPosIdxOffNarrowNode *node = new (C) leaPPosIdxOffNarrowNode();
      node->_bottom_type = _leaf->bottom_type();
      return node;
    }
